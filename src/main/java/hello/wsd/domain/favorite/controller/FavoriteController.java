...
import hello.wsd.domain.favorite.service.FavoriteService;
...
public class FavoriteController {
    ...
    @Operation(summary = "상점 즐겨찾기 추가", description = "특정 상점을 단골 상점으로 등록합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "즐겨찾기 추가 성공"),
            @ApiResponse(responseCode = "400", description = "이미 추가된 상점 / 본인 상점", content = @Content(schema = @Schema(implementation = SwaggerErrorResponse.class))),
    ...
    @Operation(summary = "상점 즐겨찾기 수 조회", description = "특정 상점의 총 단골 수를 조회합니다.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "내 단골 상점 목록 조회", description = "내가 등록한 단골 상점 목록을 페이징하여 조회합니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "조회 성공")
    })
